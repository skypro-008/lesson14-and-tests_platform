...
import main
import solution
from ttools.skyprotests.tests import SkyproTestCase
from ttools.skyprotests.tests_mixins import DataBaseTestsMixin
...
class WherePlaysTestCase(SkyproTestCase, DataBaseTestsMixin):
    ...
    def test_is_artist_in_query(self):
        self.assertRegex(main.sqlite_query, ".%Renée Zellweger%",
                         (r'%@Проверьте, что правильно указали артиста'
                          ' в условиях запроса'))

    def test_is_tv_show_in_query(self):
        self.assertRegex(main.sqlite_query, ".TV Show",
                         (r'%@Проверьте, что выбираете только сериалы'))
